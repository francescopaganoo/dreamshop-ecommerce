  ...
  try {
    ...
    if (paymentIntent.status === 'succeeded') {
      ...
      try {
        ...
        console.log(`[PAYMENT-REQUEST] Idempotency check: cercando ordini esistenti per PI ${paymentIntent.id}`);
        ...
        if (existingOrdersResponse.data && Array.isArray(existingOrdersResponse.data)) {
          ...
          if (existingOrder) {
            console.log(`[PAYMENT-REQUEST] IDEMPOTENCY: Ordine #${existingOrder.id} già esistente per PI ${paymentIntent.id}, skip creazione`);
        ...
        console.log(`[PAYMENT-REQUEST] Idempotency check passed: nessun ordine esistente per PI ${paymentIntent.id}`);
