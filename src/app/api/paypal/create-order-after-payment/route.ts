...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Gestione errori WooCommerce
      if (wooError && typeof wooError === 'object' && 'response' in wooError) {
        const axiosError = wooError as { response?: { data?: { code?: string; message?: string; data?: { new_draft_order_id?: number } } } };
        const errorData = axiosError.response?.data;

        // Se l'errore è relativo a un coupon non valido e il pagamento è già stato effettuato,
        // ritenta senza il coupon
        if (errorData?.code === 'woocommerce_rest_invalid_coupon') {
          try {
            // Rimuovi i coupon dai dati dell'ordine
            const orderDataWithoutCoupon = {
              ...orderDataToSend,
              coupon_lines: []
            };

            const retryResponse = await api.post('orders', orderDataWithoutCoupon);
            const retryOrder = retryResponse.data;

            if (!retryOrder || typeof retryOrder !== 'object' || !('id' in retryOrder)) {
              throw new Error('Risposta non valida dal secondo tentativo');
            }
            ...
            const wooOrder = retryOrder as WooOrder;

            // Aggiungi una nota che spiega il problema del coupon
            try {
              await api.post(`orders/${wooOrder.id}/notes`, {
                note: `Ordine pagato tramite PayPal. ID transazione PayPal: ${transactionId}. NOTA: Il coupon non è stato applicato perché non più valido al momento della creazione dell'ordine.`,
                customer_note: false
              });
            } catch (noteError) {
              console.error('Errore nell\'aggiunta della nota all\'ordine:', noteError);
            ...
            return NextResponse.json({
              ...
              orderId: wooOrder.id,
              status: wooOrder.status || 'processing',
              warning: 'Coupon non applicato perché non più valido'
            });

          } catch (retryError) {
            console.error('Errore anche senza coupon:', retryError);
            // Se fallisce anche il secondo tentativo, continua con il fallback dell'ordine bozza
          }
        }

        // Se c'è un ordine bozza creato da WooCommerce, proviamo a usarlo
        if (errorData?.data?.new_draft_order_id) {
          const draftOrderId = errorData.data.new_draft_order_id;

          try {
            // Aggiorna l'ordine bozza a processing e set_paid
            await api.put(`orders/${draftOrderId}`, {
              status: 'processing',
              set_paid: true
            });

            // Aggiungi la nota PayPal
            await api.post(`orders/${draftOrderId}/notes`, {
              note: `Ordine pagato tramite PayPal. ID transazione PayPal: ${transactionId}`,
              customer_note: false
            });
            ...
            console.error('Errore nell\'aggiornamento dell\'ordine bozza:', updateError);
          }
        }
      }
